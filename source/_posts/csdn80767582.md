...
*   概率是非负的：对于每个事件`A`，![P(A) \ge 0](../img/2e1cb0917c4cb7e28034cc0db045b120.png)。
*   整个空间的概率为 1：![P(\Omega)= 1](../img/afa331a6bf17babdac42bc4268eae590.png)。
...
![](../img/5ec440fcb0c7e0a59d7c421750456fed.png)
...
![A \cap B = \phi](../img/462853b2b85490341ff597b5396809a1.png)

### 第三个公理，加法规则

在有限结果空间的背景下，公理表明：

*   如果`A`和`B`是互斥事件，那么 ![P(A \cup B) = P(A) + P(B)](../img/be3cae932e3256f97c1c669ac3fdfa60.png)。

你将在练习中表明这个公理蕴涵着更一般的东西：

*   对于任何固定的`n`，如果 ![A_1, A_2, \ldots, A_n](../img/f833b2c44a132eec5a1904c51af24405.png) 是互斥的（也就是对于所有 ![i \ne j](../img/d5ed6059504d8e50b96518c6d7d7f70d.png)，![A_i \cap A_j = \phi](../img/63bdde178fc22d90db68c9a8d741c651.png)），那么：

    ![P\big{(} \bigcup_{i=1}^n A_i \big{)} = \sum_{i=1}^n P(A_i)](../img/dc43a45b74bd6e70e92ea3761e01eb04.png)
...
所以 ![B \subseteq A](../img/e3e6134fa8b750cf91cc80914e92d606.png)，那些主修数据科学但不是 CS 的人是`A`与`B`的差：

![A - B = A \cap \bar B](../img/a88ebfa2fbdfdf8115e4b75bbf985cbe.png)

其中 ![\bar B](../img/1fbe2c374b8c5a224f97ccd4f812f1d0.png) 是`B`的补集。差是右侧浅蓝色的圆圈。
...
![](../img/6cee027d7f5e100a2091a23eb27049d4.png)

这个学生在浅蓝色的差中的几率是多少呢？ 如果你回答“50% - 40% = 10%”，你是对的，你的直觉说概率的行为就像区域一样。他们是这样。 事实上，这个计算是从可加性的公理出发的，我们也通过查看这些区域来受它们启发。

### 减法规则

假设`A`和`B`是事件，![B \subseteq A](../img/e3e6134fa8b750cf91cc80914e92d606.png)。那么 ![P(A - B) = P(A) - P(B)](../img/d1d82d8cdbbbb324f368ab4e8d0d0ea3.png)。

证明。由于 ![B \subseteq A](../img/e3e6134fa8b750cf91cc80914e92d606.png)，

![A = B \cup (A - B)](../img/5f07a395cfe7d76b94540da8be76ca9b.png)

这是个不相交集合，根据加法公理：

![P(A) = P(B) + P(A - B)](../img/cab35ef3f5c41451c023739dfc219c2f.png)

所以，

![P(A - B) = P(A) - P(B)](../img/d1d82d8cdbbbb324f368ab4e8d0d0ea3.png)
...
对于任何事件`B`，![P(\bar B) = 1 - P(B)](../img/34fd2106d4bbce3f72673c1202227ce1.png)。

证明。 下面的维恩图显示了要做什么。 在减法公式取`A = Ω`，记住第二个公理 ![P(Ω)= 1](../img/49c4d7bf38727bb7f9577827e871717d.png)。或者，在这种特殊情况下为减法规则重新取参数。
...
![](../img/cd1be72a73970efb8205679192ee9f41.png)
...
一枚硬币被抛出`n`次，以使所有 ![2^n](../img/73bae2ba1e7e9f242083209e680a4a84.png) 种可能的正反面序列是等可能的。
...
让`A`成为事件“我们得到至少一个正面和至少一个反面”。问题要求`P(A)`。因为 ![\bar A](../img/9fa3d255211a4f75ed9b87f38274c141.png) 是事件“序列的所有元素都相同”，所以我们有：

![P(\bar A) = \frac{2}{2^n} = \frac{1}{2^{n-1}}](../img/4e3a598e99fdef58d083aa71840183ef.png)

根据补集规则：

![P(A) = 1 - \frac{1}{2^{n-1}}](../img/2835da8c200796ecd70e15c6c8d4d5e5.png)

请注意，随着`n`变大，答案趋于 1。随着大量的投掷，你几乎肯定可以看到正面和反面。

## 示例 2：骰子的 12 次投掷的最大值

一个骰子投掷了 12 次，所以所有 ![6^12](../img/263b68a2b869f663332252455026cb7e.png) 个点数序列是等可能的。将 12 个投掷的最大值定义为 12 个点数中出现的最大值。 例如，序列`354222143351`的最大值是 5。

问题 1。最大值小于 5 的概率是多少？

答案 1。关键是观察事件“最大值小于 5”与事件“所有 12 个面都小于 5”相同。 为了发生这种情况，12 个点数中的每一个都必须具有四个值 1 到 4 之一。所以：

![P(\text{maximum is less than 5}) = \frac{4^{12}}{6^{12}}](../img/f00d7b5358e5f79eae22a541f31a3cbe.png)
...
![P(\text{maximum is less than 4}) = \frac{3^{12}}{6^{12}}](../img/7c396bf1f1a107cd0c7fcfb3d46b390b.png)
...
![\begin{align*} P(\text{maximum is equal to 4}) &= P(\text{maximum is less than 5}) - P(\text{maximum is less than 4}) \\ &= \frac{4^{12}}{6^{12}} - \frac{3^{12}}{6^{12}} \end{align*}](../img/faadec347b7b09b48a3c9f607e9dc72a.png)
...
所以如果 ![p = P(\text{second digit is greater than the first})](../img/5b4df3afc6285f4d57fa084cca7321e1.png)，加法规则表明：

![\begin{align*} 1 &= P(\text{first digit is greater than the second}) + P(\text{the two digits are equal}) + P(\text{second digit is greater than the first}) \\ &= p + \frac{10}{100} + p \end{align*}](../img/50731816c37ead01885482cf3f4bb0a6.png)

因为有 10 对相等的数字：`00, 11, 22, ..., 99`。现在求解`p`：

![p = \frac{1 - 0.1}{2} = 0.45](../img/b4c8745bc68767fc9667fbb1cd6ac9a8.png)
...
![](../img/77646698a9140eb0e46cf90dd44231a5.png)
...
![P(\text{GR}) = \frac{#{\text{GR}}}{6} = \frac{1}{6}](../img/6653358930e76af879b82a1d8b0ef666.png)

简单的计算包含更有趣的东西。 注意：

![P(\text{GR}) = \frac{1}{6} = \frac{1}{3} \times \frac{1}{2}](../img/e073167f4d5f9371b9e799f07efcca75.png)

第二个因子 1/2 是什么？要理解这一点，只要看看`G`排在第一位的偶对。 其中，只有一个的下一张牌是`R`。乘积的第二个因子是：

![\frac{#{\text{GR}}}{#{\text{GR, GB}}} = \frac{1}{2}](../img/dacb9d42199e601db56263b26bdbf499.png)
...
![\begin{align*} P(\text{GR}) &= \frac{\#\{\text{GR}\}}{6} \\ \\ &= \frac{\#\{\text{GR, GB}\}}{6} \times \frac{\#\{\text{GR}\}}{\#\{\text{GR, GB}\}} \\ \\ &= P(\text{first card G}) \times P(\text{second card R} \mid \text{first card G}) \end{align*}](../img/94b59d81ec93c35610cdfef8e862d1c1.png)
...
![P(B \mid A) = \frac{P(AB)}{P(A)}](../img/77172358cd746af9d027873f693ea163.png)
...
![P(AB) = P(A) \cdot P(B \mid A)](../img/5cd9ef402f37ecc9ccf7d9847d4dfc7c.png)

注意答案是“小部分的小部分”。`A`和`B`都发生的几率小于`A`发生的几率 - 事件上条件越多，发生的概率就越小。

由于`AB ⊆ B`，你知道`P(AB)`小于`P(B)`。你也检查一下：

![P(AB) = P(B) \cdot P(A \mid B)](../img/6c41627658587185e64a0da8a95d1664.png)
...
![\frac{4}{52} \cdot \frac{3}{51}](../img/e69d7b6d7d6c922dc24e03918321789b.png)

问题 3。如果带放回地发牌，问题 1 和问题 2 的答案如何改变？

答案 3（究竟谁带放回地发牌？仅仅在概率班里面……）在抽出第二张牌之前，你放回了牌。在这个假设下，你每次都从完全相同的牌组上进行抽取，所以：

![P(\text{second card is an ace} \mid \text{first card is an ace}) = \frac{4}{52}](../img/4346dbe8a358add1794b0882ba6efc87.png)

无论第一张牌是什么，答案都是一样的。同时：

![P(\text{both aces}) = \frac{4}{52} \cdot \frac{4}{52}](../img/2a57f709ee8b9e6b24fb0cc8fdfe39c2.png)
...
![\frac{32791}{32791 + 9037} = 78.4\%](../img/098debbfb30ee8bc1df25d08f5c22d16.png)

这与条件概率的定义一致，即你应该计算：

![\begin{align*} P(\text{woman} \mid \text{99 years old}) &=  \frac{P(\text{woman and 99 years old})}{P(\text{99 years old})} \\ \\ &= \frac{32791\big{/}318857056}{(32791 + 9037)\big{/}318857056} \\ \\ &= \frac{32791}{32791 + 9037} = 78.4\% \end{align*}](../img/baf75a28642adc54370afbe5cc5c21b7.png)
...
![P(\text{first ball black}) = \frac{b}{b+w}](../img/fcf629464d5f36e5347ae92b5617a123.png)

问题 2：第二个抽出的球是黑色的几率是多少？

答案 2。你自然而然会想，第一个球是什么，所以根据那个球的颜色进行划分，然后相加。基本方法再次发挥作用。

![\begin{align*} P(\text{second ball black}) &= P(WB) + P(BB) \\ \\ &= \frac{w}{b+w} \cdot \frac{b}{b+w+d} ~+~ \frac{b}{b+w} \cdot \frac{b+d}{b+w+d} \\ \\ &= \frac{wb + b^2 + bd}{(b+w)(b+w+d)} \\ \\ &= \frac{b(b+w+d)}{(b+w)(b+w+d)} \\ \\ &= \frac{b}{b+w} \end{align*}](../img/aa17105093e271297a955b9cadd834d6.png)
...
P(second ball black∣first ball black)=b+db+w+d P ( second ball black ∣ first ball black ) = b + d b + w + d 

问题 4：给定第二个球是黑色的，第一个球是黑色的几率是多少？

答案 4，这种“时光倒流”的条件概率不易读出。这是除法规则的所在。

![\begin{align*} P(\text{first ball black} \mid \text{second ball black}) &= \frac{P(BB)}{P(\text{second ball black})} \\ \\ &= \frac{\frac{b}{b+w} \cdot \frac{b+d}{b+w+d}}{\frac{b}{b+w}} \\ \\ &= \frac{b+d}{b+w+d} \end{align*}](../img/76bbd5d0f67ad3670c581b01699b10bc.png)
...
![](../img/b67c40cf6f3e5cba0b486c59cea817fa.png)

为了解决这个问题，我们将使用除法规则。 让`D`为患者拥有疾病的事件，并且在一些数学符号被滥用的情况下，让`+`成为患者测试结果为阳性的事件。 那么我们要找的是`P(D | +)`。按照除法规则，

P(D∣+)=P(D and +)P(+)=0.004⋅0.990.004⋅0.99+0.996⋅0.005=44.3% P ( D ∣ + ) = P ( D  and  + ) P ( + ) = 0.004 ⋅ 0.99 0.004 ⋅ 0.99 + 0.996 ⋅ 0.005 = 44.3 % 
...
一般来说，如果整个结果空间可以划分为事件 ![A_1, A_2 \ldots , A_n](../img/5419e1d392149ed4cf2677e6ef023ccb.png)，`B`是一个正概率事件，那么对于每个`i`，

![\begin{align*} P(A_i \mid B) &= \frac{P(A_iB)}{P(B)} ~~~~ \text{(division rule)} \\ \\ &= \frac{P(A_iB)}{\sum_{j=1}^n P(A_j B)} ~~~~ \text{(the }A_j\text{'s partition the whole space)} \\ \\ &= \frac{P(A_i)P(B \mid A_i)}{\sum_{j=1}^n P(A_j)P(B \mid A_j)} ~~~~ \text{(multiplication rule)} \end{align*}](../img/e7235708fe56a2299c505f4b0a478450.png)

这种计算称为贝叶斯规则，是一个环境下的除法规则的应用，其中事件 ![A_1, A_2 \ldots , A_n](../img/5419e1d392149ed4cf2677e6ef023ccb.png) 可以看做“较早”阶段的结果，并且`B`是“较晚”阶段的结果。通过计算，我们可以求出给定较晚事件的，较早事件的“时光倒流”的条件概率，通过写出给定较早事件的，较晚事件的“随时间前进”的条件概率。
