...
本篇章中用到的隐写术的图片，都打包在了[http://pan.baidu.com/s/1mg1Khw0](http://pan.baidu.com/s/1mg1Khw0)，想去自己尝试一遍的话可以去下载。

最开始接触到隐写术，是看到一种叫做图种的东西，当时不懂，只说要另存为zip，然后解压出来就可以了，当时觉得特别神奇，就像发现了新大陆，然后就尝试了一下，发现可以用另存为zip的方式，用7z或者是winzip等工具打开，然后就可以看到福利了。

![enter image description here](../img/1ec65c901f8ea44cf973abd2a42f6659.png "enter image description here")
...
![enter image description here](../img/c6fee3a0a8d1d1b25b8f240c23d78ac8.png "enter image description here")

图2.png

这种类型的隐写也是比较容易被发现的，如果发现是jpg图片的话，观察文件结束符之后的内容，查看是否附加的内容，正常图片都会是FF D9结尾的。还有一种方式来发现就是利用binwalk这个工具，在kali下自带的一个命令行工具。

![enter image description here](../img/f7d4df9fba2b15eb81f149364fc1b4a2.png "enter image description here")
...
![enter image description here](../img/e43ee82189d009e0574d4bfc6edcb3ad.png "enter image description here")

图4.png

![enter image description here](../img/181450549090b5770e7008b9683a90e9.png "enter image description here")
...
![enter image description here](../img/6b45286708478f782d9f6f4a6d850cb7.png "enter image description here")

图6.png

再解一下qrcode，用在线的就可以[http://tool.chinaz.com/qrcode/](http://tool.chinaz.com/qrcode/)，得到了flag{AppLeU0}，如果是 隐写的使用了ascii的话，可以使用Stegsolve——Analyse——Data Extract来查看ascii码。
...
用一个ctf的题目作为例子吧，iscc2014中有一个题目，给了一个名为 此为gif图片.gif的文件，打开发现了报错。有的时候，会需要我们去修复图片，这对我们对于图片的文件结构要有了解。找到gif的文件格式，然后对照 这个破损的文件。Gif的图片格式文档可以查看这个链接，[http://dev.gameres.com/Program/Visual/Other/GIFDoc.htm](http://dev.gameres.com/Program/Visual/Other/GIFDoc.htm)

![enter image description here](../img/a583c02a7de5b47fe9ca3555177084f9.png "enter image description here")

图片8.png

用winhex打开，我们会发现他和普通的GIF图片不一样，头部缺少了东西，在对比一些文档，会发现是少了GIF8。

![enter image description here](../img/34ed7f6f77b0673693bf3473a1fcab41.png "enter image description here")

图片9.png

我们手动修复一下，增加GIF8。

![enter image description here](../img/2ae14214bcbfbb432533531da4066aba.png "enter image description here")
...
![enter image description here](../img/d5219ceb8cafe1305eddfaac7f19b681.png "enter image description here")
...
![enter image description here](../img/59ed09706904e1a7e1aa4998139b3fd8.png)

图片12.png

可以用pngcheck来辅助我们观察，可以看得更加清晰。pngcheck.exe -v sctf.png

![enter image description here](../img/d5137a0f666ebb6a64a667fcc688da18.png "enter image description here")

图片13.png

可以看到，正常的块的length是在65524的时候就满了，而倒数第二个IDAT块长度是45027，最后一个长度是138，很明显最后一个IDAT块是有问题的，因为他本来应该并入到倒数第二个未满的块里。

![enter image description here](../img/4fd7e64338aaecd21c526839b87bcf8b.png "enter image description here")

图片14.png

我们用winhex把这一部分异常的IDAT块给扣出来。然后就是要研究研究这个块是什么情况，发现了载体之后就是要想办法找出他的规律。观察那一 部分的数据，可以看到是16进制的78 9C开头的，百度一下分析是zlib压缩的标志。在png的百度百科里也可以查到PNG的IDAT是使用从LZ77派生的无损数据压缩算法，可以用 zlib解压。那么就尝试用zlib来解一下这段数据。Zlib的扩展阅读[http://zlib.net/](http://zlib.net/)

我们使用python来编程，先把那段数据处理一下，保存成16进制的。

![enter image description here](../img/5d4cfdc5e1eeb2375542a5e89d278f48.png "enter image description here")

图片15.png

得到16进制的以方便python处理，前面的4字节是长度 然后是标志位IDAT 然后开始是数据，直到 D9 CF A5 A8是crc32校验位。 所以实际的数据是：

![t01133592fbbb79650e.png](../img/4ac5f0adcb64a170ce7db08fded4e43e.png "t01133592fbbb79650e.png")
...
![t01c9a2bd97d3e2c236.png](../img/98c3ec303147d2d69165abd742f47b28.png "t01c9a2bd97d3e2c236.png")

得到的01 串的长度是625，除以8 除以7 都无法整除，也就是说没法直接转换成ascii码。

![enter image description here](../img/86815ebbad72fe9a1541bca0dcdaf4fa.png)
...
![enter image description here](../img/c6ff8a79b66c572af678f2e26e1b6ab2.png)

图片17.png

![enter image description here](../img/023980df6cdc625b11dc7c3f409ff21f.png "enter image description here")
...
![enter image description here](../img/59571a4799a9c8940edc5980c0ab5876.png "enter image description here")
...
![enter image description here](../img/ac1104e015baee5caa97b195131499bf.png "enter image description here")

图片20.png

用linux下的命令可以进行对比，生成一个有差异的图片diff.png。compare 1.png 2.png diff.png 观察一下发现了左下角有异常，png图片像数保存是从左到右，从下往上排列的。

![enter image description here](../img/d858cf0169e4bc4a914c85b817b5ced3.png "enter image description here")
...
![enter image description here](../img/1075a3552686789e8b0c763006ca1a50.png)
...
![enter image description here](../img/1189264468d3d1b60209f67c3fa466b9.png "enter image description here")
...
![enter image description here](../img/8bfc3c2e9425ecedcdf442899e72574b.png "enter image description here")

图片24.png

![enter image description here](../img/194aff3d1f992d0e3d594a1077b0ae0d.png "enter image description here")

图片25.png

只保留00 01，这个是因为RGB的关系，只隐藏在R通道里面了，其他通道都是图片的正常像数信息，过滤掉就可以了。

![t01a17a2edc5d723bba.png](../img/1cf84835d23baf5f754e5a5d5f4ab1dd.png "t01a17a2edc5d723bba.png")

观察一下可以发现，而奇数位都是0，是多余的，把这些去除掉。直接把00 替换成0，01替换成1就可以了。

![t014cc341c41c5ac033.png](../img/c404a6b0ebc9c90f95631cf39d1c50dc.png "t014cc341c41c5ac033.png")

得到了这个之后，可以发现他的长度是184，是8的倍数，把他转换成ascii码就可以了。可以使用JPK工具来进行转换，工具的下载的链接是[http://www.wechall.net/applet/JPK_406.jar](http://www.wechall.net/applet/JPK_406.jar)。

对比2.bmp可以发现隐藏了一些00 01这些信息，把这一部分扣出来。

![enter image description here](../img/33e796cd7fe55c8a777855c5ff2a5be1.png "enter image description here")

图片26.png

JPK——binary——binary to ascii

![enter image description here](../img/48c1e5399c148122dd9026446b39b00c.png)
...
这个总结其实还是缺很多的，因为隐写术能写的东西太多了，比如jpg的冗余信息的压缩也可以隐藏进信息，还有其他的多媒体文件也可以进行隐写，例如音频文 件，视频文件等等，有很多东西可以研究。一开始是觉得隐写术特别的有趣才接触到的，就像是在藏宝寻宝一样，特别好玩，希望你们也可以感受到这种快乐。欢迎 大家和我交流，我的博客地址是[http://appleu0.sinaapp.com/](http://appleu0.sinaapp.com/)。